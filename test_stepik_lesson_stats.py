...
class LessonStatisticsTests(unittest.TestCase):
    ...
    def test_seconds_are_converted_to_minutes(self):
        ...
        text = sb.render_lesson_stats(self.client, 10)
    ...
    def test_unit_fields_do_not_override_lesson_aggregates(self):
        ...
        text = sb.render_lesson_stats(self.client, 10, course_id=20)
        for value in ("999", "888", "777"):
            self.assertNotIn(value, text)
        self.assertIn("`40`", text)

    def test_real_null_shape_is_unavailable_not_zero(self):
        data = sb.lesson_statistics(self.client, 10, course_id=20)
    ...
    def test_zero_metrics_are_available(self):
        ...
        data = sb.lesson_statistics(self.client, 10)
        self.assertEqual(data["metrics"], {
    ...
    def test_missing_lesson_is_an_error(self):
        with self.assertRaises(sb.StepikError):
            sb.lesson_statistics(self.client, 999)
        ...
        for endpoint in ("courses", "sections", "units"):
            with self.subTest(endpoint=endpoint):
                with patch.dict(self.data, {endpoint: []}):
                    with self.assertRaises(sb.StepikError):
                        sb.lesson_statistics(self.client, 10, course_id=20)
    ...
    def test_mcp_wrapper_uses_the_same_renderer(self):
        ...
        with patch.object(stepik_mcp, "_guard", return_value=None):
            with patch.object(sb, "CLIENT", self.client):
                self.assertEqual(
                    stepik_mcp.stepik_lesson_stats(10, 20),
                    sb.render_lesson_stats(self.client, 10, course_id=20),
                )
