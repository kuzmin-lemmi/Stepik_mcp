...
def stepik_cache_lesson(course_id: int, lesson_position: str) -> str:
    ...
    try:
        return json.dumps(sw.cache_lesson(sb.CLIENT, course_id, lesson_position), ensure_ascii=False, indent=2)
    except sb.StepikError as exc:
...
def stepik_cache_lesson_comments(course_id: int, lesson_position: str) -> str:
    try:
        return json.dumps(sw.cache_lesson_comments(sb.CLIENT, course_id, lesson_position), ensure_ascii=False, indent=2)
    except sb.StepikError as exc:
        return f"Ошибка Stepik Workspace: {exc}"
    except Exception as exc:
        return f"Ошибка Workspace ({type(exc).__name__}): {exc}"
...
def stepik_cache_course_text_lessons(course_id: int) -> str:
    try:
        return json.dumps(sw.cache_course_text_lessons(sb.CLIENT, course_id), ensure_ascii=False, indent=2)
    except sb.StepikError as exc:
        return f"Ошибка Stepik Workspace: {exc}"
    except Exception as exc:
        return f"Ошибка Workspace ({type(exc).__name__}): {exc}"
...
def stepik_step_comments(step_id: int) -> str:
    return _run(sb.render_step_comments, sb.CLIENT, step_id)
...
def stepik_lesson_comments(lesson_id: int) -> str:
    return _run(sb.render_lesson_comments, sb.CLIENT, lesson_id)
